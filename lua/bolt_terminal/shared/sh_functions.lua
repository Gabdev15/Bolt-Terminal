...
--[[ Count all cookers ]]
function BT.CountCooker()
    local count = 0
    for k, v in ipairs(player.GetAll()) do
        local service = BT.GetNWVariables("bt_service", v)
        if not service && BT.ServiceSystem then continue end
...
function BT.CountCookerDistributor()
    local count = 0
    for k, v in ipairs(player.GetAll()) do
        if not BT.FastFoodJobDistributor or not BT.FastFoodJobDistributor[team.GetName(v:Team())] then continue end
...
hook.Add("PlayerSwitchWeapon", "BT:PlayerSwitchWeapon:Swep", function(ply, oldWep, newWep)
    local wepIndex = oldWep:EntIndex()
    if timer.Exists("bt_eat_swep:"..wepIndex) then return true end
end)
